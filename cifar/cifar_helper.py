...
def display_stats(cifar10_dataset_folder_path, batch_id, sample_id):
    ...
    if batch_id not in batch_ids:
        print(
            'Batch Id out of Range. Possible Batch Ids: {}'.format(
                batch_ids))
        return None
...
def load_preprocess_training_batch(batch_id: int,
                                   batch_size: int,
                                   save_dir: str='data/'):
    """Load the preprocessed training data and return them in batches

    Return batches of <batch_size> or less
    """
